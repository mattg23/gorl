...
use winsafe::prelude::{kernel_Hglobal, user_Hwnd, Handle};
use winsafe::{EmptyClipboard, SetClipboardData, HGLOBAL, HWND};
...
pub(crate) fn copy_text_to_clipboard(hwnd: &HWND, text: &str) -> anyhow::Result<()> {
    ...
    let hg = HGLOBAL::GlobalAlloc(
        Some(GMEM::MOVEABLE),
        wstr.len() * std::mem::size_of::<u16>(),
    )?;
    {
    ...
    unsafe {
        let _ = SetClipboardData(CF::UNICODETEXT, hg.ptr() as _)?;
    }
...
pub(crate) fn try_set_dark_mode(hwnd: &HWND) -> anyhow::Result<()> {
    ...
    let status = unsafe {
        windows::Win32::UI::Controls::SetWindowTheme(
            w_hwnd,
            ptr_theme,
...
fn set_dark_mode_for_window(hwnd: windows::Win32::Foundation::HWND, is_dark_mode: bool) -> bool {
    ...
    unsafe {
        ...
        let res = windows::Win32::Graphics::Dwm::DwmSetWindowAttribute(
            hwnd,
            DWMWA_USE_IMMERSIVE_DARK_MODE,
            std::ptr::addr_of!(value) as _,
            size as u32,
        );
...
pub(super) fn get_function_impl(library: &str, function: &str) -> Option<*const c_void> {
    ...
    let module =
        unsafe { windows_sys::Win32::System::LibraryLoader::LoadLibraryA(library.as_ptr()) };
    if module == 0 {
        return None;
    }

    unsafe { windows::core::imp::GetProcAddress(module, function.as_ptr()) }
